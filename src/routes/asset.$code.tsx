...
function AssetDetail() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<Asset | null> => {
      return assetDatabase.getAssetByCode(code);
    },
  });
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card">
        <div className="container mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <Badge className="ml-auto" variant={data.status === "Active" ? "default" : "secondary"}>
            {data.status}
          </Badge>
        </div>
      </header>
      <main className="container mx-auto px-6 py-6 max-w-3xl">
        <div className="border rounded-lg bg-card p-5">
          ...
          <Row
            label="Serial Number"
            value={<span className="font-mono">{data.serial_number}</span>}
          />
          <Row label="Purchase Date" value={data.purchase_date} />
          <Row
            label="Purchase Price"
            value={data.purchase_price != null ? `₹ ${data.purchase_price}` : null}
          />
